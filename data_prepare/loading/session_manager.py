...
def get_session_id() -> str:
    ...
    if "user_uuid" in st.session_state:
        ...
        # Force sync back to URL if missing (prevents loss during page navigation)
        if url_uid != current_id:
            st.query_params["uid"] = current_id
            st.rerun()  # Force update URL immediately
        return current_id
        
    # 3. If missing in session but present in URL (recovery)
    if url_uid:
