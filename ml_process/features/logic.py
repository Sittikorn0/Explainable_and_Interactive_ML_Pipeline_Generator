...
def detect_leakage(df: pd.DataFrame, target_col: str) -> list[str]:
    ...
    for col in df.columns:
        ...
        # 2. จำนวน unique values เท่ากัน + มี bijective mapping กับ target (one-to-one)
        if df[col].nunique() == y.nunique() and df[col].nunique() <= 20:
            try:
                col_str = df[col].astype(str)
                y_str   = y.astype(str)
                # ตรวจ bijective mapping: แต่ละค่าของ col map ไปยัง y ค่าเดียว และ vice versa
                unique_pairs = set(zip(col_str, y_str))
                if len(unique_pairs) == col_str.nunique() == y_str.nunique():
                    warnings.append(
