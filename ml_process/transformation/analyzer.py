"""
ml_process/transformation/analyzer.py
วิเคราะห์ข้อมูลแล้วสร้าง recommendations พร้อมคำอธิบายเหตุผล
...
def analyze_encoding(df: pd.DataFrame, target_col: str) -> list[dict]:
    ...
    for col in cat_cols:
        ...
        results.append({
            ...
            "options":     ["one_hot_encoding", "label_encoding", "drop_column"],
            "reason":      reason,
            "warning":     warning,
...
def analyze_scaling(df: pd.DataFrame, target_col: str) -> dict:
    ...
    return {
        "recommended":  recommended,
        "options":      ["standard_scaler", "minmax_scaler", "robust_scaler", "no_scaling"],
        "reason":       reason,
